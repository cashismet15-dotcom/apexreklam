...
function TaskMiniRow({ task, todayIso }: { task: ClientTaskWithCustomer; todayIso: string }) {
  return (
    ...
    <Card className="gap-0 py-0">
      ...
      <CardContent className="flex max-h-96 flex-col overflow-y-auto p-0">
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => <TaskMiniRow key={task.id} task={task} todayIso={todayIso} />)
        )}
      </CardContent>
    </Card>
  )
 ...
 * görev sessizce kaybolmaz — her satırda kalan süre ya da kaç gün gecikti yazar.
 */
