...
/**
 * Evolution API (WhatsApp) ile konuşan ince bir sarmalayıcı — Yakamoz'un kendi
 * WhatsApp hattı için, Apex'in ana hattından (src/lib/whatsapp.ts, Meta Cloud
 * API) bağımsız.
 *
...
function isConfigured(): boolean {
  return Boolean(
    process.env.YAKAMOZ_EVOLUTION_API_URL &&
      process.env.YAKAMOZ_EVOLUTION_API_KEY &&
      process.env.YAKAMOZ_EVOLUTION_INSTANCE_NAME
  )
}
...
export async function sendWhatsAppMessage(phone: string, text: string): Promise<SendResult> {
  if (!isConfigured()) {
    return {
      ok: false,
      error:
        "Yakamoz WhatsApp (Evolution API) henüz yapılandırılmadı (.env.local içinde YAKAMOZ_EVOLUTION_API_URL / YAKAMOZ_EVOLUTION_API_KEY / YAKAMOZ_EVOLUTION_INSTANCE_NAME boş).",
    }
  }

  const baseUrl = process.env.YAKAMOZ_EVOLUTION_API_URL!.replace(/\/$/, "")
  const instance = process.env.YAKAMOZ_EVOLUTION_INSTANCE_NAME!
  ...
  try {
    const response = await fetch(`${baseUrl}/message/sendText/${instance}`, {
      ...
      headers: {
        ...
        apikey: process.env.YAKAMOZ_EVOLUTION_API_KEY!,
      },
      body: JSON.stringify({
